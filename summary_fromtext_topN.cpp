...
void Keyword(string text, vector<string> &result,vector<double> &weight,int topn) {
...
void Keysentence(string text, vector<string> &result,vector<double> &weightsentence,int topn)
{
...
int main(int argc, char *argv[])
{
    if (argc != 5)
    {
        cout << "Usage: " << argv[0] << " <input_file> <method> <out_file>" << endl;
    ...
    if (method == 1)
    {
        ...
        while(getline(fin, line_word))
        {
            //++m;
            //startTime = clock();
            Keyword(line_word,result_word,weight,topn);
    ...
    if (method == 2)
    {
        ...
        while(getline(fin, line_sentence))
        {
            //++j;
            //startTime = clock();
            Keysentence(line_sentence,result_sentence,weightsentence,topn);
